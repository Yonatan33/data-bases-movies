...
# *********** Query2 - top movies by revenue ***********
def query_2():
    query = f'''select md.title, mp.revenue - mp.budget as profit
                from movie_details as md, movie_profits as mp
                where md.movie_id = mp.movie_id
                order by (profit) desc
                limit 20
                '''
    answer = send_execute(query)
    for title, profit in answer:
        print(f'title: {title} | Profits: {str(profit // 1000000) + "M$" if profit > 1000000  else str(profit // 100000) + "K$"}')
